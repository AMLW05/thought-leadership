# Enterprise Technology Training Experience

Comprehensive end-user training programs for enterprise technology platform adoption and organizational change management across multiple institutions and platforms.

---

## Training Experience Overview

**10+ years delivering enterprise technology training** across K-12 public schools, community colleges, and multi-institutional systems including:

### Digital Bytes Training Series (FTCC, 2022-2025)
Monthly professional development workshop series at Fayetteville Technical Community College.

**Program Scope:**
- 100+ workshops delivered over 3 years
- 200+ faculty and staff trained
- Monthly recurring series format
- Multi-tiered training approach (quick-start guides, step-by-step tutorials, deep-dive workshops)

### Cumberland County Schools (CCS) Training Programs (Previous Role)
Enterprise platform trainer for K-12 public school system.

**Platforms Trained:**
- **Canvas LMS** - Learning management system training for teachers and administrators
- **WebEx** - Synchronous meeting platform training for virtual instruction and administrative meetings
- Concurrent training programs for both platforms during distance learning transition

**Organizational Context:**
- K-12 public school system serving multiple schools and departments
- End-user base: Teachers, administrators, support staff
- Change management for rapid platform adoption during distance learning implementation

### Multi-Platform Learning Management System Expertise

**Course Building & Platform Administration Experience:**
- **Blackboard Learn** - Traditional Blackboard course development and administration
- **Blackboard Ultra** - Next-generation Blackboard interface migration training
- **Google Classroom** - K-12 classroom management platform implementation
- **Canvas LMS** - Institutional implementation and ongoing training (K-12 and higher education)

**Demonstrates:**
- Platform-agnostic training capability
- Experience with platform migrations and transitions
- Multi-organizational training delivery (K-12 schools + community colleges + statewide system)
- Synchronous and asynchronous platform expertise

---

## Available Materials

### 1. Digital Bytes November 2022
**File:** [Digital_Bytes_November_2022.docx](Digital_Bytes_November_2022.docx)

**Session Focus:** Monthly training workshop materials
**Topics Covered:**
- Canvas LMS advanced features
- Technology integration strategies
- Faculty onboarding resources

**Training Approach:**
- Hands-on demonstrations
- Step-by-step guided practice
- Follow-up support resources

---

### 2. Digital Bytes Series 2022-2023
**File:** [Digital_Bytes_Series_2022-2023.docx](Digital_Bytes_Series_2022-2023.docx)

**Complete Series Documentation** for academic year 2022-2023

**Topics Covered Throughout Series:**
- Canvas LMS training (basic through advanced)
- Quality Matters course design standards
- AI integration for instructional design
- HyFlex teaching modality implementation
- Accessibility compliance (WCAG standards)
- Student engagement technologies
- Assessment design and delivery

**Delivery Format:**
- Monthly 60-minute workshops
- Recorded sessions available on-demand
- Quick-reference handouts and job aids
- Follow-up individual coaching available

---

## Training Program Outcomes

**Measurable Impact:**
- Reduced technology adoption time for faculty by 40%
- 95% participant satisfaction rating
- Increased Canvas LMS feature utilization across institution
- Enabled department-wide AI integration adoption
- Supported HyFlex modality rollout to 50+ courses

**Change Management Success:**
- Facilitated smooth transition to new LMS features
- Reduced IT support ticket volume for common issues
- Created sustainable peer-to-peer support network
- Established repeatable training model for future rollouts

---

## Training Methodology

**Multi-Tiered Approach:**

1. **Quick-Start Guides** (5-10 minutes)
   - One-page visual reference
   - Step-by-step with screenshots
   - Common use cases

2. **Step-by-Step Tutorials** (15-30 minutes)
   - Detailed written instructions
   - Video demonstrations
   - Practice exercises

3. **Deep-Dive Workshops** (60+ minutes)
   - Live hands-on training
   - Question and answer sessions
   - Advanced use cases
   - Individual coaching

**Enterprise Training Best Practices:**
- Just-in-time training delivery
- Role-based content customization
- Ongoing support and resources
- Continuous feedback collection
- Iterative content improvement

---

## Demonstrates Key Competencies

**For ITS Innovation Strategist Role:**
- ✅ End-user training and change management
- ✅ Enterprise platform adoption strategies
- ✅ Multi-tiered training program development
- ✅ Stakeholder communication and support
- ✅ Documentation and resource creation
- ✅ Program sustainability and scalability

---

## Usage & Licensing

**Creative Commons BY-NC-SA 4.0**

Government organizations and educational institutions may adapt these training materials with proper attribution for non-commercial purposes.

**Citation:**
```
Digital Bytes Training Series by Angela Westmoreland, M.Ed.
Fayetteville Technical Community College (2022-2025)
Available at: https://github.com/AMLW05/thought-leadership
Licensed under CC BY-NC-SA 4.0
```

---

## Contact

For consultation on developing enterprise training programs or change management strategies:

**Angela Westmoreland, M.Ed.**
- Email: westmora@faytechcc.edu | awestmoreland05@gmail.com
- LinkedIn: [linkedin.com/in/angela-westmoreland-m-ed](https://www.linkedin.com/in/angela-westmoreland-m-ed-93676b313/)
- Portfolio: [amlw05.github.io/innovation-portfolio](https://amlw05.github.io/innovation-portfolio/)
